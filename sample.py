...
st.markdown(f"""
    ...
    section[data-testid="stSidebar"] label {{
        color: {PRIMARY_COLOR} !important;  /* Dark Green */
        font-weight: 700;
    }}
    ...
    if st.session_state.get('ngo_verified', False):
        ...
        if not df.empty:
            ...
            for _, row in df.iterrows():
                ...
                qty_left = int(row['quantity'])
                if qty_left > 0:
                    cols = st.columns([2,3,2,2])
                    cols[0].write(f"{row['organization']}")
                    cols[1].write(f"{row['menu']}")
                    cols[2].write(f"{qty_left}")
                    pick_qty = cols[3].number_input("", min_value=1, max_value=qty_left, step=1, key=f"pick_{donation_id}")
                    pick_btn = cols[3].button("Pick", key=f"btn_{donation_id}")
                    if pick_btn:
                        success, msg, new_qty = update_quantity(donation_id, -pick_qty)
                        if success:
                            log_pickup(donation_id, st.session_state.get('ngo_id', 'unknown'), pick_qty)
                            st.markdown(f"<p style='color:black; font-weight:bold;'>🎉 Picked {pick_qty} meals. Remaining: {new_qty}</p>", unsafe_allow_html=True)
                            st.session_state.donor_df = get_all_donations()

# ----- ANALYSIS -----
elif menu == "Analysis":
    st.markdown("""<div style="color:black; font-family: 'Helvetica', Arial, sans-serif;">""", unsafe_allow_html=True)
...
# ----- REWARDS -----
    ...
    conn = sqlite3.connect(DB_PATH)
    ngo_df = pd.read_sql_query("SELECT ngo_id as NGO, SUM(picked_quantity) as Points FROM pickups GROUP BY ngo_id", conn)
    conn.close()
